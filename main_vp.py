...
def inference_worker(q, input_ids, sampling_params):
    llm_id = q.get()
    ray.get(llm_id.generate.remote(input_ids, sampling_params))
    q.put(llm_id)

def batched_inference_worker(q, input_ids, sampling_params):
    llm_id = q.get()
    ray.get(llm_id.batched_generate.remote(input_ids, sampling_params))
    q.put(llm_id)

def inference(llm_ids, tokenizer, requests):

    # clear the num tokens and put into queue
    q = Queue()
    for llm_id in llm_ids:
        print(ray.get(llm_id.clear_num_tokens.remote()))
        q.put(llm_id)
        
    pool = ThreadPool(len(llm_ids))
    
    ## start the time profile
    ...
    for request in tqdm(requests):
        ...
        while True:
            if not q.empty():
                pool.apply_async(inference_worker, args=(q, input_ids, sampling_params))
                break
    ...
    return end_time - start_time, sum([ray.get(llm_id.get_num_tokens.remote()) for llm_id in llm_ids])

def batched_inference(llm_ids, tokenizer, requests_by_output_len):
    ...
    for output_len, requests in tqdm(requests_by_output_len.items()):
        ...
        while True:
            if not q.empty():
                ...
                pool.apply_async(batched_inference_worker, args=(q, input_ids, sampling_params))
                break
...
if __name__=="__main__":
    ...
    if args.continuous_batch == 1:
        ...
        elapsed_time, total_num_tokens = batched_inference(llm_ids=llm_ids, tokenizer=tokenizer, requests_by_output_len=requests_by_output_len)
    else:
        elapsed_time, total_num_tokens = inference(llm_ids=llm_ids, tokenizer=tokenizer, requests=requests)
